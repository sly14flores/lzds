angular.module('ids-module', ['ui.bootstrap','bootstrap-modal','x-panel-module','pnotify-module','block-ui','school-year','window-open-post','jspdf-module','module-access']).factory('form', function($http,$timeout,$compile,bootstrapModal,xPanel,pnotify,blockUI,schoolYear,printPost,jspdf,access) {
	...
	function form() {
		...
		function list(scope) {
			...
			scope.print = {};
			scope.print.sy = scope.filter.school_year.school_year;
			scope.print.level = scope.filter.level.description;
		...
		self.generateIds = function(scope) {
			...
			if (!access.has(scope,scope.module.id,scope.module.privileges.generate_ids)) return;			
		...
		self.print = function(scope,enrollment) {
			...
			if (!access.has(scope,scope.module.id,scope.module.privileges.print_ids)) return;
			...
			var pageContent = function(data) {
				...
				doc.setFontSize(12);
				doc.setFontType('bold');
				doc.myText("List of Students",{align: "center"},0,182);

				doc.setFontSize(11);
				doc.setFontType('bold');
				doc.text(50, 200, 'Level: '+scope.print.level);
				doc.text(810, 200, "SY: "+scope.print.sy);
			...
			var header = [
				{title: "LRN", dataKey: "lrn"},
				{title: "School ID", dataKey: "school_id"},			
				{title: "Fullname", dataKey: "fullname"},
				{title: "Gender", dataKey: "gender"},
				{title: "Birthdate", dataKey: "birthdate"},
			...
			//
			doc.autoTable(header, scope.enrollees, {
				// tableLineColor: [189, 195, 199],
				// tableLineWidth: 0.75,
				addPageContent: pageContent,				
				margin: {top: 210, left: 50, bottom: 80},
				tableWidth: 'auto',
				styles: {
					lineColor: [75, 75, 75],
					lineWidth: 0.50,
